...
async function init() {
    ...
    saveLocalStorage('movie', movieLists);
    moviebanner();
}

// *** 진우 : 카드 클릭하면 id가 local storage에 저장
function cardClick(id) {
    saveLocalStorage('clickedID', id);
...
darkButton.addEventListener("click", () => changeMode(darkButton, lightButton, banner, newBackgroundImage));
lightButton.addEventListener("click", () => changeMode(lightButton, darkButton, banner, newBackgroundImage))
...
//     const printStarIndex = Math.floor(average / 2); // 평점은 8.6 이런식으로 들어와서 Math.floor를사용해 내려주고 별점은 5점만점으로 2를 나눠주었다.
